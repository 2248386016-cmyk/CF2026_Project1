...
\pagestyle{fancy}\fancyhf{}\lhead{CF2026 Project 1}\rhead{动态沪深300量化研究平台}\cfoot{\thepage}
...
\centering
\vspace*{2.0cm}
{\Huge\bfseries 动态沪深300多因子与机器学习策略研究\par}
\vspace{0.6cm}
{\Large 从基础因子框架到Ridge、LightGBM与逐日成交回测\par}
\vspace{1.6cm}
\begin{minipage}{0.88\textwidth}
\textbf{摘要}\quad 本文搭建了一个从Tushare时点数据、清洗与复权、动态资产池、因子诊断、模型训练到逐日成交和可视化审计的完整量化投研平台。基础实验在沪深主板中构建低波动、短期反转和Amihud非流动性三因子组合，以周频与月频对照及波动率目标检验成本和风险。自主拓展将研究对象收敛为历史动态沪深300，加入估值、质量、动量等10个行业与市值中性化因子，以2020--2022训练、2023--2024验证、2025冻结测试的扩展窗口训练Ridge与LightGBM，并使用逐日开盘成交器处理现金、停牌、涨跌停和费用。验证期选定的126日相对强弱状态切换策略在2025年扣费后累计收益25.51\%、年化25.40\%、最大回撤11.02\%、Sharpe 1.545；同期沪深300累计17.66\%、年化17.58\%。该结果仅是一次冻结样本外证据，不构成未来收益保证。

\vspace{0.7cm}
\textbf{关键词：}动态沪深300；多因子；Ridge；LightGBM；逐日回测；交易成本；Qlib
\end{minipage}
\vfill
...
\section{研究问题与系统框架}
\subsection{研究目标}
本项目的目标不是寻找样本内收益最高的参数，而是建立一个\key{完整、正确、可复现、可扩展}的研究平台。基础层回答三个逻辑不同的因子能否在交易限制和费用下形成可执行组合；拓展层进一步回答Ridge和LightGBM能否在历史动态沪深300中形成严格时间隔离、可逐日成交且相对基准有增量的策略。
...
\subsection{研究边界}
基础实验研究上交所和深交所主板A股；模型拓展只使用每个历史时点已生效的沪深300成分股。本文未把收益率达标作为模型正确性的标准，而是优先检查唯一键、公告日和成分时点、标签退出日、费用对账、资产池变化和重复运行一致性。

\subsection{主要结果概览}
研究形成两层证据：基础层从7个候选因子中选出低波动、短期反转与Amihud非流动性，月频组合取得28.34\%年化收益，15\%波动率目标将年化波动降至17.99\%；拓展层使用动态沪深300、10个中性化因子和Ridge/LightGBM，最终状态切换策略在2025冻结测试中累计收益25.51\%，相对沪深300高7.85个百分点。后文同时保留基础任务证据和模型拓展的时间隔离、逐日执行与网页审计结果。
...
\section{策略构建与基础回测}
\subsection{组合与成交规则}
综合因子每日按截面排名，调仓时选择前20只，基础组合目标等权。信号使用调仓日前一交易日收盘后数据，下一交易日开盘成交。开盘涨停不买、开盘跌停不卖、停牌或零成交量不交易，未成交旧仓继续保留。退市后仍无法卖出的持仓按零回收率核销，以避免使用陈旧价格虚增净值。
...
\section{自主拓展：组合与风险}
\subsection{设计与方法}
拓展固定月频三因子Top 20股票集合，仅改变组合层，比较等权、10\%/15\%/20\%波动率目标、完整协方差矩阵的ERC风险平价、8\%目标权重上限及组合方案。风险估计使用形成日及以前60个交易日，至少40个有效观测，并对协方差矩阵轻度收缩。波动率目标股票仓位为
\[
...
10\%目标获得最高Sharpe和最低回撤，但平均现金比例38.74\%，收益牺牲较大；15\%目标平均现金约17.06\%，在收益保留和风险下降之间更均衡。ERC及8\%目标上限未改善Sharpe，说明本样本的主要风险改善来自总股票暴露控制，而非Top 20内部重分配。8\%是调仓目标而非每日实际权重保证；价格变化或交易受限可能造成事后超限。

\section{模型拓展：动态沪深300与逐日回测}
\subsection{时点数据、因子与训练设计}
...
网页研究工作台直接读取上述审计CSV，允许切换状态策略、Ridge、LightGBM及沪深300，并同步显示净值、回撤和指标。网页不重新计算绩效，因此报告、网页和演示使用同一组冻结数字。

\section{结论、局限与复现}
\subsection{主要结论}
本文得到四点描述性结论：其一，低波动、短期反转和流动性三个维度可以形成逻辑互补的基础信号；其二，月频比周频更适合该信号，表现为更低换手、成本和回撤；其三，15\%波动率目标能在收益保留与风险下降间取得平衡；其四，动态沪深300模型中，验证期冻结的状态切换策略在2025扣费后跑赢基准，但回撤11.02\%略高于沪深300的10.49\%，不能据此保证未来继续产生超额收益。
...
自主拓展 & 第7--8节风险控制、动态沪深300、Ridge/LightGBM与逐日执行 & 风险拓展、模型审计、Qlib诊断和网页\\
复现包与研究报告 & 第9节入口、环境、快照、小样本及校验值 & README、一键入口、清单和离线小样本\\
\bottomrule
\end{tabularx}
\end{table}
