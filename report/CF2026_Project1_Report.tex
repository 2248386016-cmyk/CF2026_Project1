...
\pagestyle{fancy}\fancyhf{}\lhead{CF2026 Project 1}\rhead{A股多因子选股与风险控制}\cfoot{\thepage}
...
\centering
\vspace*{2.0cm}
{\Huge\bfseries 基于A股沪深主板的多因子选股与风险控制研究\par}
\vspace{0.6cm}
{\Large 2020--2025年月频三因子策略、调仓频率对照及波动率目标拓展\par}
\vspace{1.6cm}
\begin{minipage}{0.88\textwidth}
\textbf{摘要}\quad 本文搭建了一个从Tushare原始数据、清洗与复权、动态资产池、因子诊断、组合构建到含成本回测的完整量化投研框架。研究对象为2020--2025年沪深主板A股，使用历史名称构造逐日ST状态，并在上市、退市、停牌和涨跌停约束下模拟交易。本文从7个候选因子中选择20日低波动、5日反转和20日Amihud非流动性，形成Top 20三因子组合。周频与月频对照显示，月频策略年化收益28.34\%、Sharpe 1.214、最大回撤30.04\%，换手与显式成本显著低于周频。自主拓展进一步比较波动率目标、权重限制和等风险贡献组合；15\%波动率目标在收益保留与风险控制之间较均衡，年化收益24.23\%、年化波动17.99\%、Sharpe 1.296。研究同时保留风险平价未改善Sharpe的结果，并将结论限定为当前历史样本上的描述性证据。

\vspace{0.7cm}
\textbf{关键词：}A股；多因子；动态资产池；Rank IC；交易成本；波动率目标；风险平价
\end{minipage}
\vfill
...
\section{研究问题与系统框架}
\subsection{研究目标}
本项目的目标不是寻找样本内收益最高的参数，而是建立一个\key{完整、正确、可复现、可扩展}的研究平台，并回答三个问题：第一，逻辑不同的因子在沪深主板是否具有稳定的截面排序信息；第二，这些信息在交易限制和费用下能否形成可执行组合；第三，调仓频率和组合风险控制能否解释并改善策略的成本、回撤和稳定性。
...
\subsection{研究边界}
研究市场为上交所和深交所主板A股，不含创业板、科创板和北交所。本文未把收益率达标作为模型正确性的标准，而是优先检查唯一键、未来信息、费用对账、资产池变化和重复运行一致性。

\subsection{主要结果概览}
在统一数据、资产池和成本口径下，研究形成三项核心证据：因子层从7个候选因子中选出低波动、短期反转与Amihud非流动性；策略层月频组合取得28.34\%年化收益、1.214 Sharpe和30.04\%最大回撤；拓展层15\%波动率目标将年化波动降至17.99\%、最大回撤降至25.21\%，同时保留24.23\%年化收益。后文分别给出定义、诊断、成交路径与复现证据。
...
\section{结论、局限与复现}
\subsection{主要结论}
本文得到三点描述性结论：其一，低波动、短期反转和流动性三个维度可以形成逻辑互补的综合信号；其二，月频比周频更适合当前信号，表现为更低换手、成本和回撤；其三，15\%波动率目标进一步提高风险调整后收益，是当前较均衡的主方案。风险平价没有改善Sharpe的结果被完整保留。

\subsection{局限与下一步}
未建模冲击成本、滑点和100股整手约束；现金收益设为0；历史ST依赖名称区间；退市残余持仓按零回收；未进行行业与市值中性化；60日风险模型存在估计误差；传统t统计量未修正重叠标签、自相关和多重搜索。历史结果不能保证未来表现。下一步可加入行业/市值中性化、成交量参与率、现金利息、滚动样本外选择和更稳健的协方差估计。

\subsection{复现信息}
项目提供测试、下载、清洗、资产池、因子、诊断、回测、对照、拓展和最终复现包的一键入口。原始数据共有9,503个文件并逐项记录SHA-256；另提供10只股票、57个交易日的离线小样本。最终清单覆盖65个源码/配置/文档文件、74个证据输出以及DuckDB数据库。当前目录不是Git仓库，故以逐文件SHA-256作为代码版本标识。
...
自主拓展 & 第7节波动率目标、权重限制与风险平价 & 风险拓展结果、成本及稳定性比较\\
